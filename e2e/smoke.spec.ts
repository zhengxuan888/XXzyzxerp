...
async function login(page: import("@playwright/test").Page, account = username, accountPassword = password) {
  const loginResponse = await page.request.post("/api/auth/login", {
    data: { username: account, password: accountPassword },
  });
  if (!loginResponse.ok()) {
    throw new Error(`登录接口失败 (${loginResponse.status()}): ${await loginResponse.text()}`);
  }
...
test("商品图片完成安全上传、预览、404 占位与删除", async ({ page }) => {
  ...
  await expect(page.getByLabel("上传附件")).toBeVisible();
  ...
  const uploadResponse = await page.request.post("/api/mvp/attachments", {
    multipart: {
      targetType: "PRODUCT",
      targetId: productId,
      file: { name: uploadName, mimeType: "image/png", buffer: png },
    },
  });
...
test("附件拒绝伪造类型、超限文件和无权限上传", async ({ page }) => {
  ...
  await page.goto(`/admin/products/${productId}`);
  await expect(page.getByLabel("上传附件")).toBeVisible();
  const rejectedUpload = await page.request.post("/api/mvp/attachments", {
    multipart: {
      targetType: "PRODUCT",
      targetId: productId,
  ...
  expect(await rejectedUpload.json()).toEqual(expect.objectContaining({
    error: expect.objectContaining({ code: "FILE_SIGNATURE_MISMATCH" }),
  }));
