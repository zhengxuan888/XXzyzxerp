...
| PostgreSQL 并发超卖集成测试 | 通过 | 真实数据库双请求门禁 2/2；重复 5 轮共 10/10；完整浏览器回归 26/26 |
...
| 并发超卖 | `updateMany` 条件扣减 + Serializable 事务 + 有限冲突重试 + 唯一幂等流水 | `e2e/inventory-concurrency.spec.ts` 真实 PostgreSQL 双请求测试 |
...
- 已完成库存调整接口的真实 PostgreSQL 双请求并发验证；订单提交/预留链路仍建议在预发布环境增加高并发压测与故障注入。
