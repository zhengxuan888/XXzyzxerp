...
本机已安装 Docker Desktop，并完成 PostgreSQL 容器启动、初始 migration、虚构 Seed、真实登录和核心页面浏览器验收。真实 PostgreSQL 双请求并发门禁已经补齐：库存为 1 时两个独立扣减只有一个成功且余额不为负；相同幂等键并发提交只产生一条流水并只扣减一次。
...
| PostgreSQL 并发超卖集成测试 | 通过 | 库存调整与订单预占均覆盖真实数据库双请求；各重复 5 轮共 20/20；完整浏览器回归 28/28 |
...
| 并发超卖 | `updateMany` 条件扣减 + Serializable 事务 + 有限冲突重试 + 唯一幂等流水 | `e2e/inventory-concurrency.spec.ts` 与 `e2e/order-reservation-concurrency.spec.ts` 真实 PostgreSQL 双请求测试 |
...
| 物流敏感字段直链泄露 | 物流单号、轨迹、维护和附件分别鉴权；详情页与 API 使用相同目标 Scope | `e2e/shipment-field-permissions.spec.ts` 动态授权、撤销与直链测试 |
...
- 已完成库存调整及订单提交/预留链路的真实 PostgreSQL 双请求并发验证；预发布环境仍建议增加更高并发压测与故障注入。
- 已完成桌面端核心页面基础浏览器验收；仍需补充移动端布局、403/404、关键表单提交和完整业务数据闭环。
- 客户、商品、订单、库存、物流、费用、收件箱、公告、文档、审批、考勤和请假均已使用统一服务端分页响应；新模块仍须遵守相同协议。
...
## 2026-07-31：资源中心与软件资产收口

### 已完成

- 新增组织范围隔离的资源资产模型：资源分类、资源状态、生命周期动作、资源资产、软件授权资料和生命周期事件；所有组织、部门、站点、角色与菜单关系均由数据库驱动。
- 新增 `/admin/resources` 和 `/admin/software-assets`。两者复用一份受权限保护的业务台账，软件资产仅是分类专属视图，不复制数据。
- 资源中心支持服务端关键词、分类、状态、部门筛选与稳定分页；支持创建、详情、编辑、配置化流转、软归档和生命周期历史。
- 资源配置页支持新增、编辑、排序、启停分类/状态/流转动作；稳定编码不可修改。资源账号标识默认脱敏，查看和维护均有独立 Action。
- 通过乐观版本锁和数据库 `CHECK` 约束防止并发流转的数量覆盖、可用数量越界和软件席位越界。并发验收已验证两个分配请求只有一个成功，另一个返回 `409`。

### 尚未完成

- 资源成本台账（维修、保险、租金）、借用协议附件、到期/低库存通知待办、完整历史分页与导入导出模板不在本批次内。
- 本批没有迁移或复制旧 ERP 资源数据；后续需要业务负责人选择和清洗后，才可进入导入流程。

### 边界

本轮未部署、未 Push、未连接旧生产库、未使用真实员工或第三方账号。资源迁移 `20260731005439_resource_center_foundation` 含有历史生成的无关索引重命名；上线前必须在独立预发布副本做迁移演练并确认基线，不能直接用于旧生产库。

### 收口验证与回归修复

- `pnpm run test`：43 个测试文件 / 154 项通过；资源输入、范围计划、版本冲突和账号标识脱敏均有单元覆盖。
- `pnpm exec playwright test --project=chromium`：33 / 33 通过；包含库存与资源并发、订单状态机、跨角色菜单/直链/API 矩阵、物流字段脱敏、分页、附件、收件箱与资源配置入口。
- `pnpm run ts-check`、`pnpm run lint`、`pnpm run prisma:validate`、`pnpm run build` 和 `pnpm exec prisma migrate status` 均通过；本地数据库的 41 个迁移为 up to date。
- 浏览器回归中发现并修复：仪表盘快捷入口的展示元数据被误判为未支持权限条件，导致已获 `order.read` 的员工菜单中缺少订单页。现在仅认可受控的展示元数据，其余未知条件仍失败关闭。
- 业务负责人可凭 `role.read` 只读查看角色定义；全局角色注册表的新增与修改仍要求 `system.configuration.manage` 的 `ALL` Scope。两个财务配置页也已改为由统一 AppShell 提供唯一主内容区，避免嵌套 `<main>`。
