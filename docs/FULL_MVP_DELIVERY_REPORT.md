...
本机没有 Docker/PostgreSQL，因此数据库 migration 实际执行、Seed 登录、浏览器交互和真实 PostgreSQL 并发事务测试尚未执行，不能宣称本地完整演示已经验收。
...
| Docker/PostgreSQL 启动 | 未执行 | 本机未安装 `docker` 命令 |
| Migration/Seed 实库执行 | 未执行 | 无可用本地 PostgreSQL |
| 浏览器页面操作验收 | 未执行 | 缺少已启动数据库与 Seed 会话 |
...
- 需要安装 Docker Desktop 或提供本地 PostgreSQL，执行 migration、Seed、登录和完整演示链路。
- 需要在真实 PostgreSQL 上运行两个并发下单/预留请求，证明只有一个成功并核对余额、预留和流水。
- 需要用浏览器逐页验收登录、移动端基础布局、控制台错误、空态、403/404 和关键表单。
...
1. 安装/启用 Docker，执行 `docker compose up -d`。
2. 运行 migration、Seed 和数据库集成/并发测试。
3. 启动本地站点，完成浏览器核心闭环和响应式验收。
