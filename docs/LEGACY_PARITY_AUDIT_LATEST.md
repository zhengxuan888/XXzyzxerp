...
| Excel 批量录单 | 旧页 `order-entry/page.tsx:549-618,895-920`；支持 Excel 选择、解析和批量订单 | V2 订单页有模板配置，但未发现等价的订单批量导入 API/确认写入闭环 | P0 缺口；必须保留逐行预览、错误行隔离、确认后写入 |
...
- [ ] Excel 批量导入支持模板、预览、逐行错误、确认写入、失败行不写入。
- [ ] 智能地址解析覆盖旧字段，并保留人工核对。
