...
4. 若预检文件或模板有误，有 `finance.statement_import.cancel` 的人员填写原因后取消该 `PREVIEWED` 批次；取消不是删除，需重新上传并预检。
5. 有 `finance.statement_import.confirm` **和** `finance.statement.create` 的人员复核零警告、零错误的预检批次。
6. 系统在一个可串行化事务内生成 `DRAFT` 结算单与明细。
7. 对账、审批、过账和付款仍在独立流程中按其各自 Action 执行；本模块不会自动进行这些动作。
...
- 只有 `PREVIEWED` 批次可取消。`IMPORTING`、`IMPORTED` 与已取消批次均不能恢复或覆盖；取消与确认采用同一状态条件的原子更新，因此并发时只会有一方成功。
- 已取消批次退出“活跃源文件”唯一约束，修正模板或文件后可以重新上传同一 SHA-256 文件生成**新**预检批次；旧批次、原件和取消原因保留用于审计。
...
| `finance.statement_import.cancel` | 取消尚未确认的预检，并填写原因；不删除原件或历史记录 |
...
- 模板版本、预检人、确认人、取消人/原因、源文件哈希、逐行结果和生成的草稿结算单均写 Audit Log。
