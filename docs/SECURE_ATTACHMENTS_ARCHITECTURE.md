...
## 2026-07-31：文档中心扩展与持久化 Scope 修复

文档中心复用同一受控存储契约，但通过专属文档内容/预览接口读取；`DOCUMENT` 不是可由通用附件路由任意访问的目标类型。

- 允许的类型扩展为 PNG、JPEG、WebP、PDF、DOCX 与 MP4；依旧必须匹配真实文件签名、声明 MIME、白名单扩展名和大小。
- DOCX 仅解析中央目录进行 OOXML 验证与压缩风险拦截，不解压用户内容；Word 页面预览仅显示本地提取的纯文本，原排版通过受控下载查看。
- 对已经持久化的附件，读取与删除绝不再使用访问者自己的部门作为回退。服务端先解析目标资源，再以附件记录中保存的业务板块/部门边界做权限判断；目标与附件边界不一致时统一返回 404。
- 文档专属接口还核验附件的 `targetType=DOCUMENT`、`targetId=document.id`、业务板块与部门一致性，并为内容读取和预览写入不含正文的审计元数据。
- 生产替换 `LOCAL_DEMO` 前仍必须完成私有对象存储、恶意软件扫描、加密、备份恢复和保留期策略；本地文件不作为生产归档。
