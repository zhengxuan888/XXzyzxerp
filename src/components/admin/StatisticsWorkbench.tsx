...
import { BarChart3, CalendarDays, ChevronDown, ChevronLeft, ChevronRight, Globe2, PackageCheck, SlidersHorizontal, UsersRound } from "lucide-react";
...
export default function StatisticsWorkbench() {
  ...
  function currentMonth() {
    const rangeEnd = new Date();
    setEnd(isoDate(rangeEnd));
    setStart(isoDate(new Date(rangeEnd.getFullYear(), rangeEnd.getMonth(), 1)));
    setPage(1);
  }
  ...
  return (
    <div className="space-y-5">
      <header className="relative overflow-hidden rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-surface)] p-5 shadow-[var(--elevation-card)] backdrop-blur-xl">
      ...
      <section className="rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-surface)] p-4 shadow-[var(--elevation-card)] backdrop-blur-xl">
        <div className="flex flex-wrap items-center gap-2">
          <span className="mr-auto inline-flex items-center gap-2 text-sm font-semibold text-slate-700"><CalendarDays size={16} className="text-amber-700" />统计周期</span>
          <Button type="button" variant="ghost" onClick={() => quickRange(1)}>今日</Button>
          ...
          <Button type="button" variant="ghost" onClick={currentMonth}>本月</Button>
          <button type="button" onClick={() => setFiltersOpen((value) => !value)} className="inline-flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 shadow-sm sm:hidden" aria-expanded={filtersOpen}><SlidersHorizontal size={15} />筛选<ChevronDown size={14} className={`transition ${filtersOpen ? "rotate-180" : ""}`} /></button>
          <Button type="button" onClick={() => void load()}>刷新统计</Button>
        </div>
        <div className={`${filtersOpen ? "grid" : "hidden"} mt-4 gap-3 border-t border-slate-100 pt-4 sm:grid sm:grid-cols-2 xl:grid-cols-4`}>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        ...
        ].map((card) => <article key={card.label} className="rounded-xl border border-[var(--glass-border)] bg-[var(--glass-surface)] p-4 shadow-[var(--elevation-card)] backdrop-blur-xl"><span className="grid size-9 place-items-center rounded-lg bg-amber-50 text-amber-700"><card.icon size={18} /></span><p className="mt-3 text-xs text-slate-500">{card.label}</p><p className="mt-1 text-2xl font-bold tabular-nums text-slate-950">{card.value}<span className="ml-1 text-xs font-medium text-slate-400">{card.suffix}</span></p></article>)}
        <article className="col-span-2 rounded-xl border border-amber-200 bg-amber-50/70 p-4 shadow-[var(--elevation-card)] sm:col-span-1"><p className="text-xs font-medium text-amber-800">COD 金额（按币种）</p><div className="mt-2 space-y-1">{data?.summary.currencyTotals.length ? data.summary.currencyTotals.map((item) => <p key={item.currency} className="font-bold tabular-nums text-slate-950">{money(item.amountCents, item.currency)}</p>) : <p className="text-2xl font-bold">—</p>}</div></article>
      </section>

      <section className="grid gap-4 xl:grid-cols-2">
        <article className="rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-surface)] p-5 shadow-[var(--elevation-card)] backdrop-blur-xl"><h2 className="font-bold text-slate-950">订单趋势</h2><p className="text-xs text-slate-500">每日有效订单数</p><div className={`mt-5 flex items-end gap-1 overflow-x-auto ${data?.daily.length ? "h-52" : "h-28"}`}>{data?.daily.length ? data.daily.map((item) => <div key={item.date} title={`${item.date}：${item.count} 单`} className="flex min-w-7 flex-1 flex-col items-center justify-end gap-2"><span className="text-[10px] text-slate-500">{item.count}</span><div className="w-full rounded-t bg-amber-500" style={{ height: `${Math.max(4, item.count / maxDaily * 150)}px` }} /><span className="rotate-[-45deg] whitespace-nowrap text-[9px] text-slate-400">{item.date.slice(5)}</span></div>) : <p className="m-auto text-sm text-slate-400">当前范围暂无订单</p>}</div></article>
        <article className="rounded-2xl border border-[var(--glass-border)] bg-[var(--glass-surface)] p-5 shadow-[var(--elevation-card)] backdrop-blur-xl"><h2 className="font-bold text-slate-950">订单国家分布</h2><p className="text-xs text-slate-500">按收件国家统计</p><div className="mt-5 space-y-3">{data?.countries.slice(0, 10).map((item) => <div key={item.countryCode} className="grid grid-cols-[52px_1fr_44px] items-center gap-3 text-sm"><span className="font-semibold">{item.countryCode === "UNKNOWN" ? "未知" : item.countryCode}</span><div className="h-2.5 rounded-full bg-slate-100"><div className="h-full rounded-full bg-slate-900" style={{ width: `${item.count / maxCountry * 100}%` }} /></div><span className="text-right text-slate-500">{item.count}</span></div>)}{!data?.countries.length && <p className="py-10 text-center text-sm text-slate-400">当前范围暂无数据</p>}</div></article>
      </section>
