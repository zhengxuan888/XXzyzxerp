...
export default function LogisticsWorkbenchSettings({
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
    ...
    {open && <div className="mt-4 space-y-4 border-t border-slate-100 pt-4">
      ...
      <div className="space-y-2"><p className="text-sm font-medium">统计卡片名称、显示、顺序与匹配规则</p><p className="text-xs text-slate-500">匹配规则用逗号分隔：STATUS:状态、EVENT:轨迹事件、TAG:售后标签，或 NO_EVENTS。留空表示使用系统队列规则。</p>{config.cards.map((card, index) => <div key={card.key} className="grid gap-2 rounded-xl border border-slate-200 p-3 lg:grid-cols-[8rem_0.8fr_1.4fr_6rem_5rem] lg:items-center"><code className="text-xs text-slate-500">{card.key}</code><input value={card.label} onChange={(event) => setConfig({ ...config, cards: config.cards.map((item, itemIndex) => itemIndex === index ? { ...item, label: event.target.value } : item) })} className="h-9 rounded-lg border border-slate-200 px-3 text-sm" /><input value={card.matches.join(", ")} onChange={(event) => setConfig({ ...config, cards: config.cards.map((item, itemIndex) => itemIndex === index ? { ...item, matches: event.target.value.split(",").map((value) => value.trim()).filter(Boolean) } : item) })} placeholder="如 EVENT:ADDRESS_ERROR, TAG:地址错误" className="h-9 rounded-lg border border-slate-200 px-3 font-mono text-xs" /><input type="number" value={card.sortOrder} onChange={(event) => setConfig({ ...config, cards: config.cards.map((item, itemIndex) => itemIndex === index ? { ...item, sortOrder: Number(event.target.value) } : item) })} className="h-9 rounded-lg border border-slate-200 px-3 text-sm" /><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={card.isVisible} onChange={(event) => setConfig({ ...config, cards: config.cards.map((item, itemIndex) => itemIndex === index ? { ...item, isVisible: event.target.checked } : item) })} />显示</label></div>)}</div>
