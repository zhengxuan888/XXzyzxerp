...
import { Download, LoaderCircle, Pencil, Plus } from "lucide-react";
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {templates.map((template) => (
          <article key={template.id} className="rounded-xl border border-slate-200 p-4">
            ...
            <span className={`mt-2 inline-flex rounded-full px-2 py-1 text-xs ${template.isActive ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{template.isActive ? "启用" : "停用"}</span>
            {canExport && (
              <a
            ...
            {canManage && <button type="button" onClick={() => setEditingId(editingId === template.id ? null : template.id)} className="ml-2 mt-3 inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700"><Pencil size={15} />编辑</button>}
            {canManage && editingId === template.id && (() => {
              ...
              return <form onSubmit={(event) => saveTemplate(event, template.id)} className="mt-4 grid gap-3 border-t border-slate-100 pt-4">
                ...
                <label className="text-sm">工作表名称<input name="sheetName" defaultValue={config.sheetName} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2" /></label>
                <label className="text-sm">导出字段<textarea name="columnLines" required rows={12} defaultValue={config.columns.map((column) => `${column.field}=${column.header}`).join("\n")} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 font-mono text-xs" /></label>
