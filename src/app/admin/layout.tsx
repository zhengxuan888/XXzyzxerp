...
export default async function AdminLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  const availableMemberships = await prisma.membership.findMany({ where: { userId: session.userId, isActive: true, OR: [{ endedAt: null }, { endedAt: { gt: new Date() } }] }, include: { businessUnit: true, role: true, department: true, site: true }, orderBy: { isPrimary: "desc" } });
  const membershipOptions = availableMemberships.map((item) => ({ id: item.id, label: `${item.businessUnit?.name || item.businessUnitId} / ${item.role?.name || "未命名角色"}${item.department?.name ? ` / ${item.department.name}` : ""}` }));
